...
def get_profile():
    try:
        ...
        # Check if profile exists for the logged-in user's email
        profile = user_profile_collection.find_one({"email": email})
        
        if profile:
            # Profile exists, return details
            return jsonify({
                'success': True,
            ...
            # Profile does not exist, return empty fields
            return jsonify({
                'success': True,
...
def update_profile():
    try:
        ...
        required_fields = ['email', 'first_name', 'last_name', 'gender', 'birthdate', 'education']
